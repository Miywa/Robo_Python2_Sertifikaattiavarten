...
def open_robot_order_website():
    """Menee urliin ja sulkee pop_upin"""
...
def order_another_bot():
    """Klikkaa tilaa uusi robootti kohtaa"""
    page = browser.page()
    page.click("#order-another")

def clicks_ok():
    """Painaa ok jotta saa pop.upin poies"""
    page = browser.page()
    page.click('text=OK')

def fill_and_submit_robot_data(order):
    """Lisää robootin tiedot"""
    page = browser.page()
    head_names = {
        "1" : "Roll-a-thor head",
...
def embed_screenshot_to_receipt(screenshot_path, pdf_path):
    """Kuvakaappaus lisätään kuittiin. 1Mikä kuva 2Mihin pdf:ääb. 3Mihin uusi tiedosto"""
    pdf = PDF()
...
def clean_up():
    """Siivous, ei ole ohjeessa. Poistaa aina turhat tiedostot"""
